...
import { ShieldCheck, CreditCard, ArrowLeft, Loader2, Lock, CheckCircle2, Plus } from "lucide-react";
  ...
  const fetchContract = async () => {
    try {
      const { data, error } = await supabase
        .from("contracts")
        .select("id, status, event_name, valor_atual, value")
        .eq("id", contractId)
        .single();
      if (error) throw error;
      setContract(data);
    } catch {
      showError("Nao foi possivel carregar o contrato para pagamento.");
    }
  };

  const handlePayment = async () => {
    if (!selectedCardId) {
      showError("Selecione ou adicione um cartao para continuar.");
      return;
    }
    setIsProcessing(true);
    try {
      const { error } = await supabase.functions.invoke("asaas-payment", {
        body: { contractId, paymentMethodId: selectedCardId },
      });
      if (error) {
        const errorMsg = await error.context?.json();
        throw new Error(errorMsg?.error || error.message);
      }
      showSuccess("Pagamento aprovado. O valor esta seguro em Escrow.");
      navigate("/app/payments");
    } catch (error: any) {
      showError(error.message || "Erro no processamento do pagamento.");
    } finally {
      setIsProcessing(false);
    }
  };

  const total = Number(contract?.valor_atual ?? contract?.value ?? artist?.base_fee ?? 0);
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        <div className="lg:col-span-2 space-y-8">
          <h1 className="text-4xl font-black text-slate-900">Finalizar Contratacao</h1>

          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="text-lg font-black text-slate-900">Escolha o Cartao</h3>
              <Button variant="ghost" size="sm" onClick={() => setIsAddDialogOpen(true)} className="text-blue-600 font-bold gap-1">
                <Plus className="w-4 h-4" /> Novo Cartao
              </Button>
            </div>
            ...
            {loadingCards ? (
              ...
              <Card className="p-8 text-center border-dashed border-2 bg-slate-50/50 rounded-[2rem]" onClick={() => setIsAddDialogOpen(true)}>
                <p className="text-slate-500 font-medium cursor-pointer">Clique para adicionar seu primeiro cartao.</p>
              </Card>
            ) : (
          ...
          <Card className="p-6 border-none bg-emerald-50 rounded-[2rem] flex gap-4 items-start">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-emerald-700 leading-relaxed">
                O pagamento fica retido em escrow. O profissional recebe apenas apos a confirmacao do evento
                ou 24h apos a data do evento.
              </p>
            </div>
          </Card>
        </div>
          ...
          <Card className="p-8 border-none shadow-xl bg-white rounded-[2.5rem] h-fit space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-bold">{contract?.event_name || "-"}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-500 font-medium">Taxa de Servico</span>
              ...
              <div className="pt-4 border-t flex justify-between items-end">
                ...
                <span className="text-3xl font-black text-blue-600">R$ {total.toLocaleString("pt-BR")}</span>
              </div>
            </div>
            <Button
              onClick={handlePayment}
              disabled={isProcessing || !selectedCardId || !contract}
            ...
            <div className="flex items-center gap-2 text-[10px] text-center text-slate-400 justify-center">
              <Lock className="w-3 h-3" /> Pagamento processado com seguranca
            </div>
          </Card>
        </div>
      </div>
