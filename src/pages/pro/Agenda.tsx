    ...
    <div className="p-8 space-y-8 max-w-4xl mx-auto">
      ...
      {loading ? <div className="flex justify-center"><Loader2 className="animate-spin" /></div> : (
        <div className="space-y-4">
          {events.length === 0 ? (
            <Card className="p-12 text-center text-slate-400 font-medium">Nenhum show para este mês.</Card>
          ) : (
            events.map((event) => (
              <Card key={event.id} className="p-6 border-none shadow-sm bg-white flex gap-6 items-center">
                ...
                <Badge className="bg-emerald-50 text-emerald-600 border-none">{event.status}</Badge>
              </Card>
            ))
          )}
        </div>
