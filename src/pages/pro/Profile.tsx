...
import { 
  Star, MapPin, Music, Edit3, Camera, CheckCircle2, Award, Save, X, Loader2, DollarSign, Upload
} from "lucide-react";
...
const ProProfile = () => {
  ...
  const handleAvatarUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      // Upload para o bucket 'avatars' (Certifique-se de criar este bucket no Supabase como PUBLIC)
      ...
      showError("Erro ao subir foto. Verifique se o bucket 'avatars' existe e é público.");
    } finally {
      setUploading(false);
    }
  };
    ...
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      <Card className="p-8 border-none shadow-sm bg-white relative overflow-hidden">
        <div className="relative flex flex-col md:flex-row gap-8 items-center">
          ...
          <Button onClick={() => setIsEditing(!isEditing)} variant={isEditing ? "outline" : "default"} className="bg-indigo-600">
      ...
      {isEditing && (
        <Card className="p-8 border-none shadow-sm bg-white space-y-6">
          ...
          <Button onClick={handleSave} className="w-full bg-indigo-600 h-12 font-bold"><Save className="w-4 h-4 mr-2" /> Salvar Alterações</Button>
        </Card>
      )}
    </div>
  );
