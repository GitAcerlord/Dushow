...
import { Wallet, TrendingUp, Clock, CreditCard, Loader2, ArrowUpRight } from "lucide-react";
...
const ProFinance = () => {
  ...
  // Saldo Disponível = COMPLETED
  const availableGross = contracts
    .filter(c => c.status === 'COMPLETED')
    .reduce((acc, curr) => acc + Number(curr.value), 0);

  // Saldo Retido = PAID (aguardando evento)
  const pendingGross = contracts
  ...
  const netAvailable = availableGross * (1 - commissionRate);

  return (
    <div className="p-8 space-y-8 max-w-6xl mx-auto">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-black text-slate-900">Financeiro</h1>
        <Badge className="bg-indigo-600 px-4 py-1.5">Comissão: {commissionRate * 100}%</Badge>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="p-8 bg-slate-900 text-white border-none shadow-2xl rounded-[2.5rem]">
          <p className="text-slate-400 text-xs font-black uppercase mb-2">Disponível para Saque</p>
          <h3 className="text-4xl font-black">R$ {netAvailable.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</h3>
        ...
        <Card className="p-8 bg-white border-none shadow-sm rounded-[2.5rem]">
          ...
          <p className="text-slate-500 text-xs font-black uppercase">Retido em Escrow</p>
          <h3 className="text-2xl font-black text-slate-900">R$ {pendingGross.toLocaleString('pt-BR')}</h3>
          <p className="text-[10px] text-slate-400 mt-2">Liberado após o show.</p>
        </Card>

        <Card className="p-8 bg-white border-none shadow-sm rounded-[2.5rem]">
          <div className="p-3 bg-emerald-50 rounded-xl w-fit mb-4"><TrendingUp className="text-emerald-600" /></div>
          <p className="text-slate-500 text-xs font-black uppercase">Volume Total</p>
          <h3 className="text-2xl font-black text-slate-900">R$ {(availableGross + pendingGross).toLocaleString('pt-BR')}</h3>
        </Card>
      </div>

      <Card className="border-none shadow-sm bg-white overflow-hidden rounded-[2rem]">
        <div className="p-6 border-b font-black text-slate-900">Histórico de Transações</div>
        <Table>
