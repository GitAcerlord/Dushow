...
const ProContracts = () => {
  ...
  const handleAction = async (id: string, newStatus: string) => {
    try {
      ...
      // REGRA DE NEGÓCIO: Atualização real no banco
      ...
      // Gamificação real: XP apenas em conclusão
      ...
      showError("Erro ao processar ação no banco.");
    }
  };
            ...
            <div className="flex-1 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Badge className={cn(
                  ...
                  contract.status === 'REJECTED' ? 'bg-red-50 text-red-600' : 'bg-slate-100 text-slate-500'
                )}>
                  {contract.status}
                </Badge>
              </div>
